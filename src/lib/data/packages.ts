...
// Category labels
export const APT_CATEGORY_LABELS: Record<string, string> = {
  build: 'Build Tools',
  vcs: 'Version Control',
  network: 'Networking',
  editors: 'Editors',
...
export const APT_PACKAGES: AptPackage[] = [
  // Build Tools
...
// Common mise tools
export const MISE_TOOLS: MiseTool[] = [
...
export function getPackagesByCategory(): Record<string, AptPackage[]> {
  const grouped: Record<string, AptPackage[]> = {};
...
// Get mise tool options as flat list
export function getMiseToolOptions(): Array<{ value: string; label: string; group: string; description: string }> {
  return MISE_TOOLS.flatMap((tool) =>
...
// Get apt package options
export function getAptPackageOptions(): Array<{ value: string; label: string; group: string; description: string }> {
  return APT_PACKAGES.map((pkg) => ({
    value: pkg.name,
    label: pkg.name,
    group: APT_CATEGORY_LABELS[pkg.category] ?? pkg.category,
